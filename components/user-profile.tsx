...
import { getDevSession, isDevModeEnabled } from "@/lib/dev-auth";
...
export default function UserProfile({ mini }: { mini?: boolean }) {
  ...
  const fetchUserData = useCallback(async () => {
    ...
    try {
      ...
      if (isDevModeEnabled() && typeof window !== 'undefined' && localStorage.getItem('dev-session') === 'true') {
        const devSession = getDevSession();
        if (devSession) {
          setUserInfo(devSession.user);
          setLoading(false);
          return;
        }
      }
      ...
      // In dev mode, fall back to dev session if real auth fails
      if (isDevModeEnabled()) {
        const devSession = getDevSession();
        if (devSession) {
          setUserInfo(devSession.user);
  ...
  const handleSignOut = async () => {
    ...
    if (isDevModeEnabled() && typeof window !== 'undefined' && localStorage.getItem('dev-session') === 'true') {
      ...
      document.cookie = 'dev-session=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT';
      router.push("/sign-in");
      return;
    }
